...
export function ItemSelector({
    ...
    const filtered = React.useMemo(() => {
        ...
        if (!q) return itemIds;
        return itemIds.filter((id) => id.toLowerCase().includes(q) || (items[id]?.displayName ?? "").toLowerCase().includes(q));
                ...
                >
                    {visibleItems.length === 0 ? (
                        ...
                        visibleItems.map((id, index) => (
                            ...
                            >
                                <Check className={cn("h-4 w-4 shrink-0", selectedItem === id ? "opacity-100" : "opacity-0")} />
                                <div className="flex min-w-0 flex-col">
                                    <span className="truncate font-medium">{items[id]?.displayName}</span>
                    ...
                    {visibleCount < filtered.length && (
                        <div className="py-2 text-center text-xs text-muted-foreground">
                            {filtered.length - visibleCount} more — keep scrolling or refine your search
                        </div>
                    )}
                </div>
